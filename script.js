let currentStep = 1;
const totalSteps = 5; // Total steps is now 5
...
function updateSelectedProductsList() {
    ...
    additionalProducts.forEach((product, index) => {
        ...
        productItem.innerHTML = `
            ...
            <button class="remove-button" onclick="removeAdditionalProduct(${index})">Удаалить</button>
        `;
...
function updateButtonsVisibility() {
    ...
    document.getElementById('back').classList.toggle('hidden', isFirstStep);
    document.getElementById('next').classList.toggle('hidden', isLastStep);
    document.getElementById('calculate').classList.toggle('hidden', !isLastStep); // Hide calculate button
}
...
function showCalculationStep() {
    // In this version, "Calculate" button on the last step will show the selected products and total.
    // You can implement the display logic here, e.g., show a summary of selected products and their total price.
    alert("Функциональность расчета стоимости будет реализована здесь. Сейчас это просто пример.");
}
